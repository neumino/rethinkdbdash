...
Error:
Cannot perform get_field on a non-object non-sequence `1`. in:
r.expr([1, 2, 3]).groupedMapReduce(function(var_1) {
    return var_1("group")
           ^^^^^         
}, function(var_2) {
    return var_2("val")
}, function(var_3, var_4) {
    return var_3.add(var_4)
}).add("Hello")
*/
It('Test backtrace for r.expr([1,2,3]).groupedMapReduce( function(foo) { return foo("group") }, function(bar) { return bar("val") }, function(left, right) { return left.add(right) }).add("Hello")', function* (done) {
    try {
        r.nextVarId=1;
        yield r.expr([1,2,3]).groupedMapReduce( function(foo) { return foo("group") }, function(bar) { return bar("val") }, function(left, right) { return left.add(right) }).add("Hello").run()
        done(new Error("Should have thrown an error"))
    }
    catch(e) {
        if (e.message === "Cannot perform get_field on a non-object non-sequence `1`. in:\nr.expr([1, 2, 3]).groupedMapReduce(function(var_1) {\n    return var_1(\"group\")\n           ^^^^^         \n}, function(var_2) {\n    return var_2(\"val\")\n}, function(var_3, var_4) {\n    return var_3.add(var_4)\n}).add(\"Hello\")\n") {
...
r.expr([1, 2, 3]).groupBy("foo", {
                          ^^^^^   
    COUNT: true
}).add("Hello")
*/
It('Test backtrace for r.expr([1,2,3]).groupBy("foo", r.count).add("Hello")', function* (done) {
    try {
        r.nextVarId=1;
        yield r.expr([1,2,3]).groupBy("foo", r.count).add("Hello").run()
        done(new Error("Should have thrown an error"))
    }
    catch(e) {
...
Error:
Cannot perform pluck on a non-object non-sequence `1`. in:
r.expr([1, 2, 3]).groupBy("foo", "bar", {
                          ^^^^^^^^^^^^   
    COUNT: true
}).add("Hello")
*/
It('Test backtrace for r.expr([1,2,3]).groupBy("foo", "bar", r.count).add("Hello")', function* (done) {
    try {
        r.nextVarId=1;
        yield r.expr([1,2,3]).groupBy("foo", "bar", r.count).add("Hello").run()
        done(new Error("Should have thrown an error"))
    }
    catch(e) {
        if (e.message === "Cannot perform pluck on a non-object non-sequence `1`. in:\nr.expr([1, 2, 3]).groupBy(\"foo\", \"bar\", {\n                          ^^^^^^^^^^^^   \n    COUNT: true\n}).add(\"Hello\")\n") {
...
*/
It('Test backtrace for r.expr([1,2,3]).groupBy("foo", "bar", r.sum("Word")).add("Hello")', function* (done) {
    try {
        r.nextVarId=1;
        yield r.expr([1,2,3]).groupBy("foo", "bar", r.sum("Word")).add("Hello").run()
        done(new Error("Should have thrown an error"))
    }
    catch(e) {
        if (e.message === "Cannot perform pluck on a non-object non-sequence `1`. in:\nr.expr([1, 2, 3]).groupBy(\"foo\", \"bar\", {\n                          ^^^^^^^^^^^^   \n    SUM: \"Word\"\n}).add(\"Hello\")\n") {
...
*/
It('Test backtrace for r.expr([1,2,3]).groupBy("foo", "bar", r.avg("Word")).add("Hello")', function* (done) {
    try {
        r.nextVarId=1;
        yield r.expr([1,2,3]).groupBy("foo", "bar", r.avg("Word")).add("Hello").run()
        done(new Error("Should have thrown an error"))
    }
    catch(e) {
        if (e.message === "Cannot perform pluck on a non-object non-sequence `1`. in:\nr.expr([1, 2, 3]).groupBy(\"foo\", \"bar\", {\n                          ^^^^^^^^^^^^   \n    AVG: \"Word\"\n}).add(\"Hello\")\n") {
