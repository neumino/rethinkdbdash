...
Pool.prototype.getConnection = function() {
  ...
  var p = new Promise(function(resolve, reject) {
    ...
    if (connection) {
      console.log('++ got connection', connection.id, self._pool.getLength(), self.getLength());
      clearTimeout(connection.timeout);
      console.log('cleartimeout 1', connection.id);
      resolve(connection);
    }
    else {
      console.log('++ did not get a connection', self._pool.getLength(), self.getLength());
...
Pool.prototype.putConnection = function(connection, shift) {
  console.log('Put connection', connection.id);
  var self = this;
  if (self._empty === true) {
    self._empty = false;
  ...
  else if ((self._extraConnections > 0) && (shift !== true)) {
  ...
  */
    ...
    clearTimeout(connection.timeout);
    console.log('cleartimeout 5', connection.id);
    var timeoutCb = function() {
      console.log('-- Fire timeoutCb: length:', self._pool.getLength(), self.getLength());
...
Pool.prototype.createConnection = function() {
  ...
  return self._r.connect(self.options.connection).then(function(connection) {
    ...
    connection.on('end', function(e) {
      ...
      */
      clearTimeout(connection.timeout);
      console.log('cleartimeout 7', self._pool.getLength(), self.getLength());
    ...
    console.log("Just created a connection");
...
Pool.prototype._expandBuffer = function() {
  console.log('!! Expanding buffer', this._pool.getLength());
