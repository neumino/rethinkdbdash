...
PoolMaster.prototype.handleAllServersResponse = function(servers) {
  var self = this;
  self._log('PoolMaster.prototype.handleAllServersResponse with '+servers.length+' servers');
  ...
  for(var i=0;i<self._pools[UNKNOWN_POOLS].length; i++) {
    ...
    self._log('Removing pool connected to: '+pool.getAddress()+' -- 1')
...
PoolMaster.prototype.deletePool = function(key) {
  ...
  self._log('Removing pool connected to: '+pool.getAddress()+' -- 2')
  pool.drain().then(function() {
    pool.removeAllListeners();
  }).error(function(error) {
...
PoolMaster.prototype.fetchServers = function(pool) {
  var self = this;
  self._r.db('rethinkdb').table('server_status')
    .union([SEPARATOR])
    .union(self._r.db('rethinkdb').table('server_status').changes()).run({cursor: true}).then(function(feed) {
    self._feed = feed;
