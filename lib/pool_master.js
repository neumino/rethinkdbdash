...
function PoolMaster(r, options) {
  ...
  self._log = helper.createLogger(options.silent || false);
...
PoolMaster.prototype.fetchServers = function() {
  ...
  self._r.db('rethinkdb').table('server_status').run({cursor: false}).then(function(servers) {
    ...
    self._log('Failed to retrieve the servers. The error returned was:');
    self._log(error.message);
    self._log(error.stack);
    setTimeout(function() {
      self.fetchServers();
    }, 1000); // Set expomential back-off
  }).catch(function(error) {
    self._log("WTF?????");
    self._log(error);
  }).finally(function() {
    self._log('PoolMaster.prototype.fetchServers - finally');
  });
}
...
PoolMaster.prototype.setServersFeed = function(pool) {
  var self = this;
  self._r.db('rethinkdb').table('server_status').changes().run({cursor: false}).then(function(feed) {
    self._feed = feed;
    feed.each(function(err, change) {
      if (err) {
        self._log('The changefeed on server_status returned an error: '+JSON.stringify(err));
    ...
    self._log('Opening the changefeed on server_status returned an error: '+JSON.stringify(error));
    setTimeout(function() {
      self.setServersFeed();
    }, 1000); //TODO Create a exponential back-off
