            ...
            if (key === UNKNOWN_POOLS) {
                for(var i=0;i<pools[key].length; i++) {
                    ...
                    for(var j=0; j<servers.length; j++) {
                        for(var k=0; k<servers[j].network.canonical_addresses.length; k++) {
                    ...
                    if (found === false) {
                        if (self._options.silent !== true) {
                            ...
                            console.error(JSON.stringify(pools[key][i].options));
                            console.error(JSON.stringify(servers));
                        }
                    }
                    else {
                        pools[key].splice(i, 1).drain().error(function(error) {
                            if (self._options.silent !== true) {
                ...
                if (found === false) {
                    ...
                    self._pools[key].drain().error(function(error) {
                        if (self._options.silent !== true) {
                            console.error('One of the removed pool could not be properly drained. The error returned was:');
                            console.error(error.message);
                            console.error(error.stack);
