        ...
        var p = new Promise(function(resolve, reject) {
            ...
            helper.loopKeys(options, function(options, key) {
                if (keepGoing === true) {
                    ...
                    else if ((key !== "timeFormat") && (key !== "groupFormat") && (key !== "binaryFormat") && (key !== "cursor") && (key !== "stream") && (key !== "highWaterMark")) {
...
Term.prototype.toReadableStream = Term.prototype.toStream;

Term.prototype.toWritableStream = function(connection, options) {
    if (this._query[0] !== termTypes.TABLE) {
        throw new Error("Cannot create a writable stream on something else than a table.");
    }
    ...
    if (helper.isPlainObject(connection) && (typeof connection._isConnection === "function") && (connection._isConnection() === true)) {
        ...
        if (helper.isPlainObject(connection) === false) {
            connection = {};
        }
        return new WritableStream(this, connection);
    }
}
Term.prototype.toTransformStream = function(connection, options) {
    if (this._query[0] !== termTypes.TABLE) {
        throw new Error("Cannot create a writable stream on something else than a table.");
    }
