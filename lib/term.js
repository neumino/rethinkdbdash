...
Term.prototype.run = function(connection, options) {
    ...
    if (helper.isPlainObject(connection) && (connection._isConnection() === true)) {
        ...
        var p = new Promise(function(resolve, reject) {
            ...
            if (connection.batch_conf) {
                query.global_optargs.push({
                    key: "batch_conf",
                    val: new Term().expr({max_els: connection.batch_conf})._self
                });
            }
...
Term.prototype.expr = function(expression, nestingLevel) {
    ...
    if (nestingLevel == null) nestingLevel = Term.prototype._nestingLevel;
    ...
    if (expression instanceof Term) {
        ...
        return new MakeArray(expression, nestingLevel-1);
    }
    else if (helper.isPlainObject(expression)) {
        return new MakeObject(expression, nestingLevel-1);
    }
    else { // Primitive
        if (expression === null) {
            return new Null();
...
Term.prototype.exprJSON = function(value, nestingLevel) {
    ...
    var result = this._exprJSON(value, nestingLevel);
...
Term.prototype._exprJSON = function(value, nestingLevel) {
    ...
    if (nestingLevel == null) nestingLevel = 100;
    if (nestingLevel < 0) throw new Error.ReqlDriverError("Nesting depth limit exceeded.\nYou probably have a circular reference somewhere")
    ...
    if (value instanceof Term) {
        ...
        for(var i=0; i<value.length; i++) {
            if (isJSON === true) {
                var temp = Term.prototype._exprJSON(value[i], nestingLevel-1);
        ...
        for(var key in value) {
            if (value.hasOwnProperty(key)) {
                if (isJSON) {
                    var temp = Term.prototype._exprJSON(value[key], nestingLevel-1);
...
function MakeArray(array, nestingLevel) {
    this.type = "MAKE_ARRAY";
    this.args = [];

    for(var i=0; i<array.length; i++) {
        this.args.push(new Term().expr(array[i], nestingLevel)._self)
    }
...
function MakeObject(object, nestingLevel) {
    this.type = "MAKE_OBJ";
    this.optargs = []
    for(key in object) {
        if (object.hasOwnProperty(key)) {
            this.optargs.push({
                key: key,
                val: new Term().expr(object[key], nestingLevel)._self
            });
        }
    }
...
function Func(func) {
    ...
    for(var i=0; i<func.length; i++) {
        ...
        if (Func.prototype.nextVarId === 9007199254740992) { // That seems like overdoing it... but well maybe...
            Func.prototype.nextVarId = 0;
        }
        else {
            Func.prototype.nextVarId++;
        }
    }
