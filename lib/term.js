...
function Term(r, value) {
    //TODO Create two fields? and add a if statement before pushing?
    var self = this;
...
Term.prototype.tableList = function() {
    ...
    query._query.push(args); //TODO Do we need to push an empty array?
    return query;
}
...
//TODO Fields?
Term.prototype.sum = function() {
    this._arity(arguments, 0, "sum", this); 
...
//TODO Fields?
Term.prototype.avg = function() {
    this._arity(arguments, 0, "avg", this); 
...
//TODO Fields?
Term.prototype.min = function() {
    this._arity(arguments, 0, "min", this); 
...
//TODO Fields?
Term.prototype.max = function() {
    this._arity(arguments, 0, "max", this); 
...
Term.prototype.expr = function(expression, nestingLevel) {
    ...
    if (expression instanceof Term) {
        ...
        for(var i=0; i<expression.length; i++) {
            args.push(new Term(this._r).expr(expression[i], nestingLevel-1)._query)
        }
        query._query.push(args);
        return query;
    }
        ...
        for(key in expression) {
            if (expression.hasOwnProperty(key)) {
                optArgs[key] = new Term(this._r).expr(expression[key], nestingLevel-1)._query;
            }
        }
        return new Term(this._r, optArgs);
    }
    else { // Primitive
        if (expression === null) {
