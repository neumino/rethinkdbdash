...
function Term(r, value, error) {
    ...
    return new Proxy(term, {
        get: function (target, prop, receiver) {
            if (target[prop] !== undefined || prop === '_error') {
...
Term.isTerm = function (term) {
    return term instanceof Term || term.__isTerm;
}
...
Term.prototype.time = function() {
    ...
    var foundArgs = false;
    for(var i=0; i<_args.length; i++) {
        if ((_Term.isTerm(_args[i])) && (_args[i]._query[0] === termTypes.ARGS)) {
            foundArgs = true;
            break;
        }
    }
...
Term.prototype.expr = function(expression, nestingLevel) {
    ...
    if (expression === null) {
    ...
    else if (Term.isTerm(expression)) {
        return expression;
    }
        ...
        if (expression === null) {
            ...
            return new Term(self._r, expression, self._error);
        }
