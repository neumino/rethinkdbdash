        ...
        var p = new Promise(function(resolve, reject) {
            ...
            helper.loopKeys(options, function(options, key) {
                if (keepGoing === true) {
                    ...
                    else if ((key !== "timeFormat") && (key !== "groupFormat") && (key !== "binaryFormat") && (key !== "cursor")) {
                        reject(new Error.ReqlDriverError("Unrecognized option `"+key+"` in `run`. Available options are useOutdated <bool>, durability <string>, noreply <bool>, timeFormat <string>, groupFormat: <string>, profile <bool>, binaryFormat <bool>, cursor <bool>"));
                        keepGoing = false;
                    }
                }
            });
                        ...
                        if (keepGoing === true) {
                            if ((key === "useOutdated") || (key === "durability") ||
                               (key === "noreply") || (key === "arrayLimit") || (key === "profile")) {
                                ...
                                setTimeout( function() {
                                    reject(new Error.ReqlDriverError("Unrecognized option `"+key+"` in `run`. Available options are useOutdated <bool>, durability <string>, noreply <bool>, timeFormat <string>, groupFormat: <string>, profile <bool>, binaryFormat: <string>, cursor <bool>"));
                                }, 0);
                                keepGoing = false;
                                return false;
                            }
...
Term.prototype.point = function(latitude, longitude) {
    // Arity check is done by r.point
    this._noPrefix(this, "point");

    var term = new Term(this._r);
    term._query.push(termTypes.POINT);
    term._query.push([new Term(this._r).expr(latitude)._query, new Term(this._r).expr(longitude)._query]);
    return term;
}
