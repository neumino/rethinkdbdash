...
r.prototype.distance = function(geometry, otherGeometry, options) {
  if (Term.prototype._fastArityRange(arguments.length, 2, 3) === false) {
    var _len = arguments.length;var _args = new Array(_len); for(var _i = 0; _i < _len; _i++) {_args[_i] = arguments[_i];}
    Term.prototype._arityRange(_args, 2, 3, 'r.distance', this);
  }

  var term = new Term(this);
  if (options !== undefined) {
    return term.distance(geometry, otherGeometry, options);
  } 
  else if (otherGeometry !== undefined) {
    return term.distance(geometry, otherGeometry);
  }
  else {
    return term.distance(geometry);
  }
}
