...
Cursor.prototype.next = function() {
    ...
    var p = new Promise(function(resolve, reject) {
        if (self._closed === true) {
            reject(new Err.ReqlDriverError("You cannot called `next` on a closed cursor."))
        }
        else if ((self._data.length === 0) && (self._canFetch === false)) {
            reject(new Err.ReqlDriverError("No more rows in the cursor"))
        }
        else {
...
Cursor.prototype._flush = function() {
    while ((this._pendingPromises.length > 0) && (this._data.length > 0)) {
...
Cursor.prototype._each = function(callback, onFinish) {
    ...
    var resolve = function(data) {
        callback(null, data);
        self.next().then(resolve).error(reject);
    }
