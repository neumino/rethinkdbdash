function ReqlDriverError(message, query, backtrace) {
    ...
    if ((query != null) && (query._self != null) && (query._self.type != null)) {
        if (generateBacktrace) {
            this.message += " after:\n"+generateBacktrace(query._self, 0, null, [-1]).str;
        }
        else {
            this.message += ".";
        }
...
function ReqlRuntimeError(message, query, frames) {
    ...
    var back = frames.backtrace.frames.map(function(frame) { return frame.pos.low; })
    queryResult = generateBacktrace(query, back, null, back);

    this.message += " in:\n"+queryResult.str+"\n"+queryResult.car;
    this.query = query;
    this.frames = frames.backtrace.frames.map(function(frame) { return frame.pos.low; });
    console.log('');
};
...
specialType = {
    DATUM: function(term, index, father, frames) {
    ...
    TABLE: function(term, index, father, frames) {
        var result = {
            str: "",
            car: ""
        }
        var _backtrace, underline, currentFrame;
        ...
        if (term.args[0].type !== "DB") {
            ...
            if (term.args) {
                for(var i=0; i<term.args.length; i++) {
                    ...
                    if (currentFrame === 1) {
                        _backtrace = generateBacktrace(term.args[i], i, term, frames)
                    }
                    else {
                        _backtrace = generateBacktrace(term.args[i], i, term, null)
                    }
                    result.str += _backtrace.str;
                    result.car += _backtrace.car
                }
            }
            ...
            if (currentFrame === 0) {
                _backtrace = generateBacktrace(term.args[0], 0, term, frames);
            }
            else {
                _backtrace = generateBacktrace(term.args[0], 0, term, null);
            }
            result.str = _backtrace.str;
            result.car = _backtrace.car;
    ...
    GET_FIELD: function(term, index, father, frames) {
        var result = {
            str: "",
            car: ""
        }
        ...
        if (currentFrame === 1) {
            _backtrace = generateBacktrace(term.args[1], 1, term, frames)
        }
        else {
            _backtrace = generateBacktrace(term.args[1], 1, term, null)
        }
        result.str += _backtrace.str;
        result.car += _backtrace.car;
    ...
    MAKE_OBJ: function(term, index, father, frames) {
        var result = {
            str: "",
            car: ""
        }
        ...
        for(var i=0; i<term.optargs.length; i++) {
            ...
            carify(result, term.optargs[i].key+": ", underline);


            if (currentFrame === i) {
                _backtrace = generateBacktrace(term.optargs[i].val, index, term, frames);
            }
            else {
                _backtrace = generateBacktrace(term.optargs[i].val, index, term, null);
            }
            result.str += _backtrace.str;
            result.car += _backtrace.car;
        }
    ...
    FUNC: function(term, index, father, frames) {
        var result = {
            str: "",
            car: ""
        };
        ...
        if ((term.args[0].length === 0) && (JSON.stringify(term).match('"type":"IMPLICIT_VAR"'))) {
            _backtrace = generateBacktrace(term.args[1], index, term)
            result.str = _backtrace.str;
            result.car = _backtrace.car;
        }
        else {
    ...
    FUNCALL: function(term, index, father, frames) {
        ...
        if (currentFrame === 1) {
            _backtrace = generateBacktrace(term.args[1], 0, term, frames);
        }
        else {
            _backtrace = generateBacktrace(term.args[1], 0, term, null);
        }
        result.str = _backtrace.str;
        result.car = _backtrace.car;
    ...
    IMPLICIT_VAR: function(term, currentFrames, frames, index, father) {
        var result = {
            str: "",
            car: ""
        }
    ...
    GROUPBY: function(term, index, father, frames) {
        var result = {
            str: "",
            car: ""
        };
        ...
        if (currentFrame === 1) {
            _backtrace = generateBacktrace(term.args[1], 1, term, frames, {noBracket: true});
        }
        else {
            _backtrace = generateBacktrace(term.args[1], 1, term, null, {noBracket: true});
        }
        result.str += _backtrace.str;
        result.car += _backtrace.car;

        if (currentFrame === 2) {
            _backtrace = generateBacktrace(term.args[2], 2, term, frames);
        }
        else {
            _backtrace = generateBacktrace(term.args[2], 2, term, null);
        }
...
function generateBacktrace(term, index, father, frames, options) {
    ...
    var _backtrace, currentFrame, underline;
    ...
    if (term) {
        if (specialType[term.type]) {
            _backtrace = specialType[term.type](term, index, father, frames, options);
            result.str = _backtrace.str;
            result.car = _backtrace.car;
        }
