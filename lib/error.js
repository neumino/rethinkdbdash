...
function ReqlRuntimeError(message, query, frames) {
    ...
    queryResult = backtrace(query, back, null, back);
...
specialType = {
        ...
        if (term.args[0].type !== "DB") {
            ...
            if (term.args) {
                for(var i=0; i<term.args.length; i++) {
                    ...
                    if (currentFrame === 1) {
                        _backtrace = backtrace(term.args[i], i, term, frames)
                    }
                    else {
                        _backtrace = backtrace(term.args[i], i, term, null)
                    }
            ...
            if (currentFrame === 0) {
                _backtrace = backtrace(term.args[0], 0, term, frames);
            }
            else {
                _backtrace = backtrace(term.args[0], 0, term, null);
            }
            result.str = _backtrace.str;
            result.car = _backtrace.car;
    ...
    GET_FIELD: function(term, index, father, frames) {
        ...
        if (currentFrame === 1) {
            _backtrace = backtrace(term.args[1], 1, term, frames)
        }
        else {
            _backtrace = backtrace(term.args[1], 1, term, null)
        }
        result.str += _backtrace.str;
        result.car += _backtrace.car;
    ...
    MAKE_OBJ: function(term, index, father, frames) {
        ...
        for(var i=0; i<term.optargs.length; i++) {
            ...
            if (currentFrame === i) {
                _backtrace = backtrace(term.optargs[i].val, index, term, frames);
            }
            else {
                _backtrace = backtrace(term.optargs[i].val, index, term, null);
            }
            result.str += _backtrace.str;
            result.car += _backtrace.car;
        }
    ...
    FUNC: function(term, index, father, frames) {
        ...
        if ((term.args[0].length === 0) && (JSON.stringify(term).match('"type":"IMPLICIT_VAR"'))) {
            _backtrace = backtrace(term.args[1], index, term)
            result.str = _backtrace.str;
            result.car = _backtrace.car;
        }
        else {
    ...
    FUNCALL: function(term, index, father, frames) {
        ...
        if (currentFrame === 1) {
            _backtrace = backtrace(term.args[1], 0, term, frames);
        }
        else {
            _backtrace = backtrace(term.args[1], 0, term, null);
        }
        result.str = _backtrace.str;
        result.car = _backtrace.car;
    ...
    GROUPBY: function(term, index, father, frames) {
        ...
        if (currentFrame === 1) {
            _backtrace = backtrace(term.args[1], 1, term, frames, {noBracket: true});
        }
        else {
            _backtrace = backtrace(term.args[1], 1, term, null, {noBracket: true});
        }
        result.str += _backtrace.str;
        result.car += _backtrace.car;

        if (currentFrame === 2) {
            _backtrace = backtrace(term.args[2], 2, term, frames);
        }
        else {
            _backtrace = backtrace(term.args[2], 2, term, null);
        }
...
function backtrace(term, index, father, frames, options) {
    var result = {
        str: "",
        car: ""
    }
