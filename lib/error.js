...
specialType = {
    ...
    MAKE_OBJ: function(term, index, father, frames, options) {
        ...
        if ((index === 0) && ((father == null) || (nonPrefix[father.type]))) carify(result, "r.expr(", underline)
            /*
        if ((index === 0) && ((father == null) || (nonPrefix[father.type]))) console.log(father); 
        if ((index === 0) && ((father == null) || (nonPrefix[father.type]))) console.log(term); 
        */
...
function makeOptargs(term, index, father, frames, options) {
    ...
    return specialType.MAKE_OBJ(optargs, index, term, frames, options);
}
           ...
           if (term.args) { //TODO Do we need that?
                ...
                for(var i=1; i<term.args.length; i++) {
                    ...
                    if ((currentFrame != null) && (currentFrame.pos != null) && (currentFrame.pos.low === i)) {
                        ...
                        backtrace = generateBacktrace(term.args[i], 0, term, null, options);
                    }
                    result.str += backtrace.str;
                    result.car += backtrace.car;
                }
                ...
                if ((Array.isArray(term.optargs)) && (term.optargs.length > 0)) {
                    ...
                    backtrace = makeOptargs(term, i, term, frames, options);
