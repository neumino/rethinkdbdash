...
specialType = {
        ...
        if (term.args[0].type !== "DB") {
            ...
            if (term.args) {
                for(var i=0; i<term.args.length; i++) {
                    ...
                    if (currentFrame === 1) {
                        backtrace = generateBacktrace(term.args[i], i, term, frames)
                    }
                    else {
                        backtrace = generateBacktrace(term.args[i], i, term, null)
                    }
            ...
            if (currentFrame === 0) {
                backtrace = generateBacktrace(term.args[0], 0, term, frames);
            }
            else {
                backtrace = generateBacktrace(term.args[0], 0, term, null);
            }
            result.str = backtrace.str;
            result.car = backtrace.car;
    ...
    GET_FIELD: function(term, index, father, frames) {
        ...
        if (currentFrame === 1) {
            backtrace = generateBacktrace(term.args[1], 1, term, frames)
        }
        else {
            backtrace = generateBacktrace(term.args[1], 1, term, null)
        }
        result.str += backtrace.str;
        result.car += backtrace.car;
    ...
    MAKE_OBJ: function(term, index, father, frames) {
        ...
        for(var i=0; i<term.optargs.length; i++) {
            ...
            if (currentFrame === i) {
                backtrace = generateBacktrace(term.optargs[i].val, index, term, frames);
            }
            else {
                backtrace = generateBacktrace(term.optargs[i].val, index, term, null);
            }
            result.str += backtrace.str;
            result.car += backtrace.car;
        }
    ...
    FUNC: function(term, index, father, frames) {
        ...
        if ((term.args[0].length === 0) && (JSON.stringify(term).match('"type":"IMPLICIT_VAR"'))) {
            backtrace = generateBacktrace(term.args[1], index, term)
            result.str = backtrace.str;
            result.car = backtrace.car;
        }
        else {
    ...
    FUNCALL: function(term, index, father, frames) {
        ...
        if (currentFrame === 1) {
            backtrace = generateBacktrace(term.args[1], 0, term, frames);
        }
        else {
            backtrace = generateBacktrace(term.args[1], 0, term, null);
        }
        result.str = backtrace.str;
        result.car = backtrace.car;
    ...
    GROUPBY: function(term, index, father, frames) {
        ...
        if (currentFrame === 1) {
            backtrace = generateBacktrace(term.args[1], 1, term, frames, {noBracket: true});
        }
        else {
            backtrace = generateBacktrace(term.args[1], 1, term, null, {noBracket: true});
        }
        result.str += backtrace.str;
        result.car += backtrace.car;

        if (currentFrame === 2) {
            backtrace = generateBacktrace(term.args[2], 2, term, frames);
        }
        else {
            backtrace = generateBacktrace(term.args[2], 2, term, null);
        }
