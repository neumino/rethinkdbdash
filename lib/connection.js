...
Connection.prototype._processResponse = function(response, token) {
    ...
    if (type === responseTypes.COMPILE_ERROR) {
        ...
        if (typeof self.resolveMap[token] === 'function') {
            ...
            if ((Array.isArray(datum)) &&
                  ((self.options[token].cursor === true) || ((self.options[token].cursor === undefined) && (self.r._options.cursor === true)))) {
    ...
    */
        ...
        if (!self.cursors[token]) { //No cursor, let's create one
            ...
            var type;
            if (Array.isArray(response.n) && (response.n.length > 0)) {
                if (response.n[0] === protodef.Response.ResponseNote.SEQUENCE_FEED) {
                    type = 'Feed';
                }
                else if (response.n[0] === protodef.Response.ResponseNote.ATOM_FEED) {
                    type = 'AtomFeed';
                }
                else if (response.n[0] === protodef.Response.ResponseNote.ORDER_BY_LIMIT_FEED) {
                    type = 'OrderByLimitFeed';
                }
                else if (response.n[0] === protodef.Response.ResponseNote.UNIONED_FEED) {
                    type = 'UnionedFeed';
                }
                else if (response.n[0] === protodef.Response.ResponseNote.INCLUDES_STATES) {
                    type = 'IncludesFeed';
                }
                else {
                    //TODO reject
            ...
            cursor = new Cursor(self, token, self.options[token], type);
            if ((self.options[token].cursor === true) || ((self.options[token].cursor === undefined) && (self.r._options.cursor === true))) {
                // Return a cursor
                if (self.options[token].profile === true) {
                    currentResolve({
                        profile: response.p,
...
Connection.prototype._send = function(query, token, resolve, reject, originalQuery, options) {
    ...
    if ((!helper.isPlainObject(options)) || (options.noreply != true)) {
        //TODO: With v0_4, we can force to send the STOP query...
...
Connection.prototype._flush = function() {
    helper.loopKeys(this.rejectMap, function(rejectMap, key) {
        rejectMap[key](new Error('The connection was closed before the query could be completed.'))
    });
    this.rejectMap = {};
    this.resolveMap = {};
}
